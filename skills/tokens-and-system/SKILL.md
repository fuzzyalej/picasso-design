...
0. Aesthetic in one line. Config (the three dials). 1. Visual theme. 2. Color palette and roles (name, token, role, rationale; Banned Colors). 3. Typography (families, scale; Banned Fonts). 4. Component stylings (buttons, cards, inputs, nav, loaders, empty, error; eight states each). 5. Hero or signature (fits viewport; banned: fake UI, invented metrics, version stamps). 6. Layout (grid-first, contained max-width). 7. Responsive (named breakpoints; test 375, 768, 1440). 8. Motion. 9. Anti-patterns (banned). Each visual section carries its own inline Banned subsection.
