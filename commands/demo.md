...
Load the picasso `taste`, `tokens-and-system`, `components`, `accessibility`, `motion`, and `unslop-copy` skills.
...
For each screen, author a self-contained HTML file in `<folder>/demo/` that imports `../tokens.css` then `../components.css`, composes real components using the shipped classes and only defined tokens, and includes loading (skeleton matching layout), empty, and error states plus a grid layout. Meet WCAG AA. These are compositions for feedback, not a mirror of real features; do not invent metrics, logos, or testimonials.
